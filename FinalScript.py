...
# Function to check if a term is a valid demonym
def is_demonym(demonym):
    ...
    try:
        ...
        # Check if the provided demonym matches any valid demonyms
        if isinstance(data, list) and len(data) > 0:
            country_demonyms = data[0].get("demonyms", {})
            if demonym in [country_demonyms.get("eng", {}).get("m"), country_demonyms.get("eng", {}).get("f")]:
                return True
        return False
    except requests.exceptions.HTTPError as e:
        # Suppress 404 errors and assume invalid demonym
        if response.status_code == 404:
            return False
        return False  # Silently handle other HTTP errors
    except requests.exceptions.RequestException:
        # Silently handle other request errors (e.g., connection timeout)
        return False
...
for country in countries:
    if is_demonym(country):
        print(f"Removed as demonym: {country}")
    else:
        filtered_countries.append(country)

# Sort entities for display
sorted_human_names = sorted(human_names)  # Alphabetical
sorted_countries = sorted(filtered_countries)  # Alphabetical
